...
## This code creates an output file calData_upper.dat which is used in transition/separation detection tool infradet_ltt.py
import cv2
import numpy as np
import os 
...
def main ():
    ...
    outfilename = 'calData_'+uplow+'.dat'
   ...
   ##Construct the pixel list and the AoA list in order to create AoA vs pixel data for each chordwise position
    ...
    ##Find the crop boundaries (sinem:fixed -> !!!! YOU NEED TO ADJUST HERE ACCORDING TO THE FIRST AND LAST POSITION YOU ENTER WHILE CCOMPILING IN TH EBEGINNING!!!
    ...
    ######### !!!!!!!!!!!!!!!!  The Offsets Here Needs to be Changed According to the Image, where the model is location on the Image etc  ######
